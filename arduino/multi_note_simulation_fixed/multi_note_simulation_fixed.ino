...
const bool fixed_iteration = true;
const int iteration_length = 400;
...
// not used if iteration length is fixed
...
void loop() {
  ...
  for (int i = 0; i < adcCount; i++) {
    for (int j = 0; j < adcSensorCounts[i]; j++) {
      if (!fixed_iteration || elapsed[i][j] >= iteration_length) {
        ...
        if (fixed_iteration) {
            // gravity is measured in adc bits per iteration
            // key and hammer speeds are measured in adc bits per iteration
            hammerSpeeds[i][j] = hammerSpeeds[i][j] - gravity;
            // update according to hammer velocity
            hammerPositions[i][j] = hammerPositions[i][j] + hammerSpeeds[i][j];
        } else {
